...
export default function UpdateCompanyPage() {
  ...
  const { company, startLoadCompany, startUpdateCompany } = useCompany();

  useEffect(() => {
    if (id) {
      startLoadCompany(id); // solo si id existe
    }
  }, [id]);
  ...
  const handleSubmit = async (
    ...
    try {
      // Call your hook here
      if (!id) return; // evita llamar si id es undefined
      await startUpdateCompany(id, values);
    } catch (error) {
      console.error(error);
    } finally {
